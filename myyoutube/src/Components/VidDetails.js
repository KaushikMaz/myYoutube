...
import { videoSearch } from './Constants'



const VidDetails = ({details,id}) => {
    
    const isDescriptionOpen=useSelector(store=>store.app.isDescriptionOpen)
    const dispatch=useDispatch()
    // console.log(details)
      
    const{channelTitle, title,description,likeCount,viewCount}=videoSearch(details,id.get("v"))
    
    // const {description}=details?.[1].snippet?.localized
    // const {likeCount,viewCount}=details?.[1].statistics
  return (
    <div>
        <div className=" font-semibold text-xl ml-[6rem] w-[44rem] m-2">{title}</div>
        <div className="flex w-[44rem]">
            <div className="ml-[6rem] m-2">{channelTitle}</div>
            <div className=" bg-slate-100 border border-1 hover:bg-slate-300 rounded-l-full pl-2 m-1 mr-0 flex items-center">
              ...
              <div className="m-2">{(likeCount > 1000000) ? Math.trunc(likeCount / 1000000) + 'M' : Math.trunc(likeCount / 1000) + 'k'}</div>
            </div>
            <div className=" bg-slate-100  hover:bg-slate-300 border border-1 rounded-r-full px-4 my-1 flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 ">
        ...
        <div className="ml-[6rem] w-[44rem] m-2 bg-slate-100 p-2 rounded-lg">
